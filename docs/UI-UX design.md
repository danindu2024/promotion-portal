...
*   **Summary Stats:** Displays counts for **Total Rows**, **Valid Rows (Green)**, and **Rows with Errors (Red)**.
    
*   **Action Buttons:**
    
*   **Submit Valid Records (Green Button):** Sends only the clean, valid rows to the Staging table.
    
*   **Download Error Sheet (Red Button):** Downloads a specific Excel file containing _only_ the rejected rows with an added "Error Message" column for offline correction.
...
*   **Section 2: Identity & Location (Split Columns)**
    ...
    *   **Right Column (Geography):**
        ...
        *   GN Division: Text Input / Autosuggest.
...
*   **Save Registry Button:**
    ...
    *   **Interaction:** On click, triggers client-side validation. If valid, send POST request to staging\_data.
...
*   _Geography:_ District > DS Division > GN Division.
    
*   _Demographics:_ Age Range, Gender.
...
*   2.2 **Visuals:** Sector Pie Chart, Age Pyramid, Maps .
    

**3.0 Data Entry Module (Agent Role)**

*   3.1 **Single Entry:** Manual Form for Individual Records.
...
*   3.2 **Bulk Upload:**
    ...
    *   3.2.3 **Error Handling:** Download "Error Sheet" functionality.
...
2.  Update staging\_data.validation\_status to 'Rejected'.
    
3.  Save the comment into staging\_data.rejection\_reason.
    

**Agent View:** The record reappears in the Agent's "Correction Queue," displaying the rejection\_reason so they know what to fix.

**5.0 Analytics & Reporting (Management Role)**

*   5.1 **Advanced Search:** Filter by District, GN, Trade, Age.
